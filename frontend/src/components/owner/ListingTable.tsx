    ...
    return (
        <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 overflow-hidden shadow-sm">
                    ...
                    <tbody className="divide-y divide-stone-100 dark:divide-stone-800">
                        {filteredProperties.length > 0 ? (
                            filteredProperties.map((p) => {
                                const coverImage = p.media?.find(m => m.is_cover)?.url || p.media?.[0]?.url || "/placeholder-property.jpg";
                                return (
                                    <tr key={p.id} className="hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors">
                                        ...
                                        <td className="p-4 font-medium text-stone-900 dark:text-white text-sm">
                                            NPR {(p.base_price / 100).toLocaleString()} <span className="text-stone-400 font-normal text-xs">/night</span>
                                        </td>
                                        <td className="p-4">
            ...
            <EditListingDrawer
                ...
                listing={properties.find(p => p.id === editingId) || null}
                onSave={handleSave}
            />
        </div >
    );
